...
def get_elapsed_time_with_message(
    ...
    >>> elapsed_time, time_unit, message = get_elapsed_time_with_message(elapsed_time)
    >>> print(f"Time elapsed since creation: {elapsed_time} {time_unit} {message}")
    '''
    # Determine time unit and message based on elapsed time
    if elapsed_time.days >= 30:
        months = elapsed_time.days // 30
        return (months, 'months', f'{months} month{"s" if months > 1 else ""} ago')
        ...
        return (minutes, 'minutes', f'{minutes} minute{"s" if minutes > 1 else ""} ago')
    else:
        return (
